...
const generateButtonEvents = (): EntriesEventsType => {
  const buttonEvents: EntriesEventsType = {
    cta: {
      ['Explore More'] : {
        onClick: (): void => {
          ...
          if (currentLocation.includes('pso-real-relief')) {
            location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-get-the-facts';
          } else location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-real-relief';
        }
      },
      ['Why a Biologic?'] : {
        onClick: (): void => {
          const locationUrl = document.location.toString().split('#')[0];
          const anchor = 'PSO Experience 1 - Next Best Action';
          document.location = locationUrl + '#' + anchor;
        }
      },
      ['See the Connection'] : {
        onClick: (): void => {
          location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-real-relief';
        }
      },
      ['Start the Quiz']: {
        onClick: (): void => {
          const locationUrl = document.location.toString().split('#')[0];
          const anchor = 'PSO Experience 1 - Engagement';
          document.location = locationUrl + '#' + anchor;
        }
      },
      ['Get Prepared'] : {
        onClick: (): void => {
          const locationUrl = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-cosentyx-experience';
          const anchor = 'PSO Experience 3 - Engagement';
          document.location = locationUrl + '#' + anchor;
        }
      },
      ['Start Tracking']: {
      ...
      ['Start Prepared'] : {
        onClick: (): void => {
          location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-appointment-prep';
        }
      },
      ['Explore COSENTYX'] : {
        onClick: (): void => {
          location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-cosentyx-experience';
        }
      },
      ['Get Started'] : {
        onClick: (): void => {
      ...
      ['Know Your Risk'] : {
        onClick: (): void => {
          location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/';
        }
      },
      ['We Can Help'] : {
        onClick: (): void => {
      ...
      ['See Your Options'] : {
        onClick: (): void => {
          location.href = 'https://nov-prospect-as-nonprod-dev.azurewebsites.net/pso-savings-options';
        }
      },
      ['Start Preparing'] : {
        onClick: (): void => {
    ...
    ctaContainer: {
      ['think-about-your-life-with-plaque-psoriasis'] : {
        ['Answered Mostly A'] : {
          onClick: (event: Event, splideElement: React.RefObject<Splide>) => {
            if(splideElement){
              splideElement.current?.go('+1');
            }
          }
        },
        ['Answered Mostly B'] : {
          onClick: (event: Event, splideElement: React.RefObject<Splide>): void => {
            if(splideElement){
              splideElement.current?.go('+2');
            }
          }
        },
      }
